...
class Gateway extends WC_Payment_Gateway
{
	...
	public function init_form_fields()
	{
		$this->form_fields = array(
			...
			'title' => array(
				...
				'default' => __('Pays.cz payment', 'woocommerce'),
				'desc_tip' => true,
			),
			'description' => array(
	...
	public function process_payment($orderId)
	{
		...
		if (!in_array($order->order_currency, array('CZK', 'USD', 'EUR'), TRUE)) {
			return array(
				'result' => 'fail',
				'redirect' => '',
			);
		...
		$this->database->insert(Plugin::TABLE_PAYMENTS, array(
			'identifier' => $merchantOrderNumber = $this->getMerchantOrderNumber($orderId),
			'status' => Plugin::STATUS_INITIATED,
			'customer' => $order->get_formatted_billing_full_name() . ', ' . $order->billing_email,
			'order' => $orderId,
			'amount' => $order->order_total . ' ' . $order->order_currency,
			'date_initiated' => date('Y-m-d G:i:s'),
		));

		$link = 'https://www.pays.cz/paymentorder?Merchant=' . Plugin::getMerchantId() . '&Shop=' . Plugin::getShopId();
		$link .= '&Currency=' . $order->order_currency . '&Amount=' . ($order->order_total * 100) . '&Email=' . $order->billing_email;
		$link .= '&MerchantOrderNumber=' . $merchantOrderNumber;
